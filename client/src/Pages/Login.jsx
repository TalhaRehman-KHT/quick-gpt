...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // ✅ FIX: use "data", not "date"
            const { data } = await axios.post(url, { name, email, password });

            // ✅ FIX: correct spelling "success"
            if (data.success) {
                setToken(data.token);
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] text-gray-500 rounded-lg shadow-xl border border-gray-200 bg-white">
                ...
                {state === "register" && (
                    <div className="w-full">
                        ...
                        <input
                            ...
                            className="border border-gray-200 rounded w-full p-2 mt-1 text-purple-700"
                            type="text"
                            required
                        />
                    </div>
                ...
                {state === "register" ? (
                    <p>
                        ...
                        <span onClick={() => setState("login")} className="text-purple-700 cursor-pointer">click here</span>
                    </p>
                ) : (
                    <p>
                        Create an account?{" "}
                        <span onClick={() => setState("register")} className="text-purple-700 cursor-pointer">click here</span>
                    </p>
                )}
